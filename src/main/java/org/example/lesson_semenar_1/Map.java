...
public class Map extends JPanel {
    ...
    private boolean checkLine(int x, int y, int line1, int line2, int len, int dot) {
        final int lenx = x + (len - 1) * line1;
        final int leny = y + (len - 1) * line2;
        if (!isValidCell(lenx, leny)) return false;
        for (int i = 0; i < len; i++) {
            if (field[y + i * line2][x + i * line1] != dot) return false;
        }
        return true;
    }
